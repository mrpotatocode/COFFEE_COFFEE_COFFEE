...
```{r lollipops, out.width="66%", fig.align="center"}
...
```{r map, out.width="66%", fig.align="center"}
...
```{r twocoffees}
kable(data %>% filter(Roaster == 'Pista') %>% filter(Coffeename %like% 'Fugi') %>% mutate_all(funs(str_to_title(.)))  %>% 
  select(Roaster, Coffeename, Country, Region, Variety, Processing, Tastingnotes), 
  caption="Different Processed Coffees", booktabs = TRUE, linesep= "") %>% 
  kableExtra::kable_styling(latex_options = c("stripped", "HOLD_position"))
```

As expressed previously, adding Region to models was desirable. However, combination frequencies (**Table \@ref(tab:freqcomboregion)**) demonstrate the limitations of a dataset this size, where very few regions had more than five frequent occurences when combined with Variety and Process. 
...
```{r tastingfrequencyplot, out.width="66%", fig.align="center"}
...
findings - we are seeing consistency, but we only know to look for things we expect
treemap shows the varieties in these itemsets that is obsevered - drawing huge conclusions is difficult
learned about the world - without knowing more levels of detail we shouldn't be making any conclusive statements
...
```{r}
library(treemap) #citethis

tree <- unite(prep_data %>% mutate_all(funs(str_to_title(.))) %>% 
  filter(Country == 'Ethiopia'), ItemSet, Country,Variety1,Processing1, sep = " + ")
