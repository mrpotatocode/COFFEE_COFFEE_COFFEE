...
cr_5 <- comp_t %>% filter(Country == 'costa rica') %>% group_by(ItemSet) %>% summarize(n = n()) %>% arrange(desc(n)) %>% top_n(5)

#push the variables into model
comp_t <- comp_t %>% filter(ItemSet %in% cr_5$ItemSet) %>% select(Variety1, Processing1, Country) %>% distinct()
...
kable(cr_pred_fin %>% arrange(ItemSet), 
  caption="Most Frequently Occurring Costa Rican Coffees and their Predictions", booktabs = TRUE, linesep= "") %>% 
  kableExtra::kable_styling(latex_options = c("stripped", "HOLD_position"))
```
