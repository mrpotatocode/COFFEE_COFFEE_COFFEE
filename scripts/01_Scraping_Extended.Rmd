...
get the roasters
```{r}
#all_roasters <- read_html("https://eightouncecoffee.ca/collections/shop-by-coffee-roaster")
## save the data
#write_html(all_roasters,"EightOunce/roasters.html")
all_roasters <- read_html("EightOunce/roasters.html")
```

```{r}
rough_roasters <- all_roasters %>% 
  html_nodes("body") %>% 
  html_nodes("div") %>% 
  html_nodes("div") %>% 
  html_nodes("main") %>% 
...
roasters <- 
tibble(raw_text = rough_roasters)
```
...
roasters 
roasters %>% 
  mutate(is_roaster = if_else(raw_text %like% "\n\n ",1,0)) %>% 
  filter(is_roaster == 1) 
```
```{r}
...
#run line first
#then save (block 2)
#then read in block 2 saved data as read html

#raw_data <- read_html("https://eightouncecoffee.ca/collections/the-library-specialty-coffee")
## save the data
#write_html(raw_data,"EightOunce/TLS.html")
raw_data <- read_html("EightOunce/TLS.html")
...
coffee_names <- our_data %>% 
  separate(raw_text, into = c("1","2","3","4","5","6","7","Name"), sep = "\\n",  remove = TRUE) %>% 
  select(Name) %>%
...
coffee_names <- coffee_names %>% 
  separate(Name, into = c("Roaster","CoffeeName"), sep = "The Library Specialty Coffee ",  remove = TRUE)
```
...
merge(coffee_names, coffee_table, by=0, all=TRUE)
