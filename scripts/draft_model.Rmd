...
```{r}
merged_data %>% select(Tastingnotes,TastingNote1,Trait1,Group1, TastingNote2,Trait2,Group2,TastingNote3,Trait3,Group3) %>% distinct() %>%  
  filter(is.na(Trait1))
```
...
tidy_kfolds <- vfold_cv(train_data)
```
...
```{r}
#TUNE PACKAGE
lr_tune <- tune_grid(lr_mod,
          lr_recipe,
          resamples = tidy_kfolds)

lr_param <- lr_tune %>% select_best("roc_auc")

lr_mod <- finalize_model(lr_mod,lr_param)
```
